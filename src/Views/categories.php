                    ...
                    <?php if (isLogin()): ?>
                        <td class="border px-4 py-2 flex justify-center">
                            <form action="/categories/edit/<?php echo escape($category["slug"]) ?>/" class="mt-3 sm:mt-0 sm:float-right text-base" method="post">
                                <div class="flex">
                                    <input type="text" name="edit" class="mr-2 rounded border px-2 text-base" placeholder="Catégorie" value="<?php echo escape($category["category"]); ?>">
                                    <button type="submit" class="w-10 h-10 bg-yellow-500 hover:bg-yellow-600 text-white"><i class="fas fa-edit"></i></button>
                                </div>
                                <span class="text-red-500 font-bold"><?php echo getError(escape($category["slug"])) ?></span>
                            </form>
                            <form action="/categories/delete/<?php echo escape($category["slug"]) ?>/" method="post">
