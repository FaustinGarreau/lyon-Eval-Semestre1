...
<h1 class="mb-8 text-2xl font-bold"><i class="fas fa-book mr-4 text-purple-900"></i>Editer un Livre</h1>
    <div class="card bg-white rounded shadow relative border-l-4 border-purple-900">
        <form action="/livres/<?php echo $book["bookSlug"]; ?>" method="post">
            <header class="p-4 flex items-center">
                <label for="title"><i class="fas fa-heading mr-4 text-purple-900"></i></label>
                <div class="flex-grow">
                    <input id="title" type="text" name="title" value="<?php echo isset($_SESSION["old"]) ? old("title") : escape($book["title"])?>" class="w-10/12 rounded border py-2 px-4">
            ...
            <div class="p-4 flex items-center border-t">
                ...
                <div class="flex-grow">
                    <input id="title" type="text" name="author" value="<?php echo isset($_SESSION["old"]) ? old("author") : escape($book["author"])?>" class="rounded border py-2 px-4 w-full" placeholder="Votre auteur">
            ...
            <div class="p-4 flex items-center border-t">
                ...
                <div class="flex-grow">
                    <select name="category">
                        <option value="<?php echo escape($book["categoryId"]); ?>">La catégorie est <?php echo escape($book["category"]); ?></option>
                        <?php
                            foreach ($categorys as $category) {
                                ?>
                                    <option value="<?php echo escape($category["id"]); ?>"><?php echo escape($category["category"]); ?></option>
                                <?php
                            }
                        ?>
                    </select>
            ...
            <div class="content border-t border-b p-4 flex-grow flex items-center">
                ...
                <div class="flex-grow">
                    <textarea name="description" id="description" rows="10" class="w-full rounded border py-2 px-4"><?php echo isset($_SESSION["old"]) ? old("description") : escape($book["description"])?></textarea>
            ...
            <div class="p-4 flex border-b items-center">
                ...
                <div class="flex-grow">
                    <input id="slug" type="text" name="slug" value="<?php echo isset($_SESSION["old"]) ? old("slug") : escape($book["bookSlug"])?>" class="w-full rounded border py-2 px-4">
            ...
            <footer class="p-4 flex justify-between items-center">
                <div class="flex">
                    ...
                    <div class="flew-grow">
                        <input type="date" class="rounded border py-2 px-4" name="date" id="date" value="<?php echo isset($_SESSION["old"]) ? old("date") : escape($book["date"])?>">
        ...
        <form action="/livres/<?php echo escape($book["bookSlug"]); ?>/delete" method="post" class="absolute top-0 right-0 mt-4 mr-4 text-white">
